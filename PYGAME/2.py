...
def print_bingo_board(arg_list, col_num = N):
...
# n x n 빙고 보드 작성 : 1차원 배열 1~36 사이의 중복되지 않는 정수

bingo_board = []

# 백슬라이스를 이용해서 이렇게 표현이 가능하다.
...
while bingo_count < 2:
    ...
    for i in range(bingo_element_num):
        if bingo_board[i] == rand_num:
            bingo_board[i] = "*"
            break
